...
```
The best model as determined by Akaike’s Information Criterion (AIC) is the saturated model, including season and distance to forest edge with an AICc value of 226.3.
...
ggplot(data = nestbox_clean, # selecting clean nest box data
  ...
  geom_line(data = model1_predictions, # using model1 predictions
            ...
            inherit.aes = FALSE) + # avoid main aesthetics
...
This figure shows observed Swift Parrot presence plotted against distance from the forest edge, separated by season (2016 and 2019). Jittered points represent individual occupancy observations from nest boxes. Predicted probabilities of Swift Parrot presence from a logistic regression model (model1) are overlaid with a 95% confidence intervals. The predictions illustrate how the likelihood of Swift Parrot presence varies with edge distance between seasons. 
...
# creating a new data frame with prediction points (0m and 900m from forest edge)
probability_data <- expand.grid(  # all combinations of edge_distance and season
  edge_distance = c(0, 900), # predictor values
  season = c("2016", "2019")  # seasons
)

# predicted probabilities and standard errors
predictions <- predict(   # model predictions
  model1,  # logistic regression model
  newdata = probability_data, # predict for each row of probability_data
  type = "response", # output on probability scale
  se.fit = TRUE   # standard errors for each prediction
)

probability_data$predicted_prob <- predictions$fit  # adding predicted probabilities to data frame
probability_data$se <- predictions$se.fit  # adding CI of predictions to data frame

# calculating 95% confidence intervals
probability_data$lower_CI <- plogis(  # lower CI
  qlogis(probability_data$predicted_prob) - 1.96 * probability_data$se
)
probability_data$upper_CI <- plogis(  # upper CI
  qlogis(probability_data$predicted_prob) + 1.96 * probability_data$se
)

print(probability_data)   # printing prediction results
